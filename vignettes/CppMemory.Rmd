...
```
## Rcpp
This tutorial will use Rcpp to write C++ code directly in R so that R users can interact with C++ code in a familiar environment.
```{r load-Rcpp}
library(Rcpp)
```
...
- [C++ Reference](https://www.w3schools.com/cpp/cpp_references.asp): 
  - A reference variable is an alias for an existing variable. It is created using the `&` operator, e.g. `int y = 3; int &x = y`
...
- [C++ Pointer](https://www.w3schools.com/cpp/cpp_pointers.asp):
  - A variable that stores the memory address as its value. It is created using the `*` operator, e.g. `int y = 3; int* x = &y;` In FIMS, we used shared pointer declared, `std::shared_ptr<int> y` which we will explore later.
  - The operator `*` can also be used to access the value the pointer points to, e.g. `*x` 
  
```{r def-reference}
cppFunction('int reference() {
  int y = 3;
  int &x = y;
  std::cout << "x is: " << x << std::endl;
  std::cout << "y is: " << y << std::endl;
  std::cout << "The memory address of x is: " << &x << std::endl;
  std::cout << "The memory address of y is: " << &y << std::endl;
...
```{r def-pointer}
cppFunction('int pointer(){
  float y = 3.1459;
  //initiate a variable x that points to the same address as y
  float* x = &y;
  std::cout << "x is equal to the address of y" << std::endl;
  std::cout << "x is: " << x << std::endl;
  std::cout <<  "The address of y is: " << &y << std::endl;
  std::cout << "*x returns the value of y: " << *x <<  std::endl;
  return 0;
}')
...
```{r pointer-update}
cppFunction('int update_pointer(){
 //initiate a variable
  float a = 3.1459;
...
```{r reassign-pointer}
cppFunction('int reassign_pointer(){
//initiate a variable
  float a = 3.1459;
  //initiate a new variable with the same value as a
  float b = a;
...
When pointers are initialized inline, e.g. `int x = 3; int* y = x`, the compiler automatically creates and manages memory for you. The user can also create and manage memory themselves using the `new` and `delete` commands. Care must be taken to manage memory properly. If you create a variable using `new` and don;t clean up after using `delete`, the program can result in **memory leaks** where the memory in the program accumulates over its run time, which can slow down or even crash your program.

```{r manage-memory}
cppFunction('int manage_memory(){
